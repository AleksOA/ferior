<?php
session_start();
function multi_network_custom_login (){
/**
 * WordPress User Page
 *
...
//            wp_safe_redirect( wp_login_url() ); // WAS
            wp_safe_redirect( home_url('login') ); // my code
            exit;
        }
...
<!--        <form class="admin-email-confirm-form" name="admin-email-confirm-form" action="--><?php //echo esc_url( site_url( 'wp-login.php?action=confirm_admin_email', 'login_post' ) ); ?><!--" method="post"> // WAS-->
        <form class="admin-email-confirm-form" name="admin-email-confirm-form" action="<?php echo esc_url( site_url( 'login?action=confirm_admin_email', 'login_post' ) ); ?>" method="post"> // my code
            <?php
            /**
...
//                wp_login_url()// WAS
    ...
    case 'retrievepassword':
        if ( $http_post ) {
//            $errors = retrieve_password(); //WAS
            $errors = custom_retrieve_password(); // my code

            if ( ! is_wp_error( $errors ) ) {
//                $redirect_to = ! empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : 'wp-login.php?checkemail=confirm'; // WAS
                $redirect_to = ! empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : 'login?checkemail=confirm'; // my code
                wp_safe_redirect( $redirect_to );
                exit;
            }
        }
...
<!--        <form name="lostpasswordform" id="lostpasswordform" action="--><?php //echo esc_url( network_site_url( 'wp-login.php?action=lostpassword', 'login_post' ) ); ?><!--" method="post"> // WAS-->
        <form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url( home_url( 'login?action=lostpassword', 'login_post' ) ); ?>" method="post">
            <p>
        ...
        <p id="nav">
<!--            <a href="--><?php //echo esc_url( wp_login_url() ); ?><!--">--><?php //_e( 'Log in' ); ?><!--</a> // WAS-->
            <a href="<?php echo esc_url( home_url('login') ); ?>"><?php _e( 'Log in' ); ?></a>
            <?php

            if ( get_option( 'users_can_register' ) ) {
                $registration_url = sprintf( '<a href="%s">%s</a>', esc_url( home_url('login?action=register') ), __( 'Register' ) );
...
//                wp_redirect( site_url( 'wp-login.php?action=lostpassword&error=expiredkey' ) ); // WAS
                wp_redirect( site_url( 'login?action=lostpassword&error=expiredkey' ) );
            } else {
//                wp_redirect( site_url( 'wp-login.php?action=lostpassword&error=invalidkey' ) ); // WAS
        ...
        if ( ( ! $errors->has_errors() ) && isset( $_POST['pass1'] ) && ! empty( $_POST['pass1'] ) ) {
            ...
            setcookie( $rp_cookie, ' ', time() - YEAR_IN_SECONDS, $rp_path, COOKIE_DOMAIN, is_ssl(), true );
//            login_header( __( 'Password Reset' ), '<p class="message reset-pass">' . __( 'Your password has been reset.' ) . ' <a href="' . esc_url( wp_login_url() ) . '">' . __( 'Log in' ) . '</a></p>' ); // WAS
            login_header( __( 'Password Reset' ), '<p class="message reset-pass">' . __( 'Your password has been reset.' ) . ' <a href="' . esc_url( home_url('login') ) . '">' . __( 'Log in' ) . '</a></p>' );
            login_footer();
            exit;
        }
        ...
        ?>
<!--        <form name="resetpassform" id="resetpassform" action="--><?php //echo esc_url( network_site_url( 'wp-login.php?action=resetpass', 'login_post' ) ); ?><!--" method="post" autocomplete="off"> // WAS-->
        <form name="resetpassform" id="resetpassform" action="<?php echo esc_url( home_url( 'login?action=resetpass', 'login_post' ) ); ?>" method="post" autocomplete="off">
...
<!--            <a href="--><?php //echo esc_url( wp_login_url() ); ?><!--">--><?php //_e( 'Log in' ); ?><!--</a> // WAS-->
    ...
    case 'register':
        if ( is_multisite() ) {
             ...
             */
//            wp_redirect( apply_filters( 'wp_signup_location', network_site_url( 'wp-signup.php' ) ) ); // WAS
            wp_redirect( apply_filters( 'wp_signup_location', home_url( 'signup' ) ) );
            exit;
        }

        if ( ! get_option( 'users_can_register' ) ) {
//            wp_redirect( site_url( 'wp-login.php?registration=disabled' ) ); //WAS
        ...
        if ( $http_post ) {
            ...
            if ( ! is_wp_error( $errors ) ) {
//                $redirect_to = ! empty( $_POST['redirect_to'] ) ? $_POST['redirect_to'] : 'wp-login.php?checkemail=registered'; // WAS
        ...
        ?>
<!--        <form name="registerform" id="registerform" action="--><?php //echo esc_url( site_url( 'wp-login.php?action=register', 'login_post' ) ); ?><!--" method="post" novalidate="novalidate"> // WAS -->
...
<!--        <form name="loginform" id="loginform" action="--><?php //echo esc_url( site_url( 'wp-login.php', 'login_post' ) ); ?><!--" method="post"> // WAS-->
       ...
       // adding username to the login input =================================================
...
add_shortcode( 'custom_login', 'multi_network_custom_login' );



//  Add existing user to the chosen blog   ============================
function custom_authenticate_add_user_to_blog($user, $username, $password) {
    $user_data = $_SESSION['user_data'];
    if(!is_wp_error( $user ) && !null){
        if($user_data) {
